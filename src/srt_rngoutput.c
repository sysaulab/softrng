...
int main(int argc, char** argv){
    ...
    sprintf(cmd, "RNG_output %s inf %llu", argv[1], seed);
    FILE* child = fopen(cmd, "r");
    while(1) { 
        buf_received = fread(buffer, sizeof(char), _SSRNG_BUFSIZE, child);
        if(buf_received != _SSRNG_BUFSIZE) {
            fprintf(stderr, "srt_rngoutput: error reading child.");
            break;
        }
        if(buf_received != fwrite(buffer, sizeof(char), _SSRNG_BUFSIZE, stdout)){
            fprintf(stderr, "srt_rngoutput: error writing stdout.");
            break;
        }
    }
    return EXIT_SUCCESS;
