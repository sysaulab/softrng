# speedy64 and the Seedy64 family

**A userspace entropy source, its mechanism, its variants, and its deployment envelope**
...
We present the Seedy64 family of userspace entropy sources, culminating in speedy64. The design harvests scheduler indeterminacy from three threads racing on a ring of shared 64-bit nodes, sampled by a thread that calls `sched_yield()` immediately before reading the ring state. The generator requires two independent ingredients, neither of which alone is sufficient: a large state space produced by the racing threads, and an unpredictable sampling instant produced by the yield. We established this through a series of control experiments in which each ingredient was removed in turn, and each removal produced a distinctive failure.

speedy64 sustains at least 12 MB/s on an Apple M1 host, roughly 14× the sustained rate of its polling predecessor on the same hardware, while providing approximately twice the conservative min-entropy per byte. Min-entropy per byte, measured with the NIST SP 800-90B non-IID estimators on one hundred independent 1 MB slices per environment, ranges from 6.638 to 7.486 on bare metal with means above 7.1. The source is portable across x86_64 and ARM64, robust to background load, and degrades monotonically under reduced concurrent execution.

The polling predecessor, by contrast, has a collapse mode: in a nine-slice sample on the same M1 host, its min-entropy bound ranged from 0.967 to 4.229 bits per byte, with the bitstring estimator binding in every slice. The timer path is conditionally unpredictable; the scheduler path is not. This is a change in kind, not merely a change in degree.

The security warnings are unchanged and non-negotiable. speedy64 is a statistical source, not a cryptographic one. Its entropy is shared with any co-resident process. It must not be used for keys, tokens, nonces, or any security-relevant output.

---

## 1. Introduction

This paper documents a single project: a userspace entropy source built on the observation that microscopic timing indeterminacy can be amplified by nonlinear mixing into macroscopic output variation. The project went through four design iterations, each of which taught us something the previous one had obscured. The final design, speedy64, is what remains after the alternatives failed.

The original Seedy64 sampled a three-thread racing ring from a fourth thread that slept for a fixed interval. The sleep was documented as an interface detail. It was not. Removing it in favour of a tight emission loop failed PractRand. Replacing it with `sched_yield()` recovered the quality and exceeded it. A series of further experiments isolated what the yield was actually doing, what the ring was actually providing, and what happens when either is removed.

The paper is structured as follows. Section 2 describes the design. Section 3 describes the mechanism, including the two-ingredient model and the evidence for each ingredient. Section 4 reports the measurements. Section 5 compares the design to its polling predecessor. Section 6 characterises the deployment envelope. Section 7 states the security position. Section 8 lists open questions. Two appendices cover the collaboration and reproduction.

A note on naming. **Seedy64** is the original polling-mode generator and the name of the parent project. **speedy64** is the yield-per-emit design documented here. **seedier64** is the informal name given to the family of failed variants; the failures are documented as controls within this paper rather than in a separate one, because the mechanism cannot be explained without them.
...
Both ingredients are necessary. Neither is sufficient. The original design happened to have both; the polling predecessor's sampling unpredictability was conditional on the timer not quantizing, which is why it has a collapse mode (§5).

### 3.6 The three-node count is not arbitrary

The two-ingredient model does not predict how many racing nodes are required, only that the state space must exceed the sampling resolution. Empirically, three nodes pass and two degrade quality. The reason is not understood. Candidate explanations include the odd-parity behaviour of the three-way XOR, the directed-cycle topology of a three-node ring, and the state-space size difference. The number is not a free parameter and the empirical observation is reproducible. It is listed as an open question.
...
All generators were run on the Apple M1 host under macOS. All assessments were run on a Debian Linux VM on the same physical host, or on bare-metal x86_64 Linux for the cross-architecture check.
...
Streams were produced by a single process writing to stdout, piped to a file. Each stream was split into 1 MB slices. Each slice was assessed independently with the NIST SP 800-90B non-IID estimators:
...
The NIST streaming test suite was used as a secondary screen on some configurations. The streaming test accumulates evidence across many 1 MB blocks and is sensitive to slow drift; the non-IID batch estimators are sensitive to local structure. The two are complementary.
...
- **Bitstring drops.** Three slices report `H_bitstring` between 0.654 and 0.787. The tight-loop failure mode reappears in a small fraction of slices.
...
| speedy64 (`--emit-yield`, `skip = 1`) | ≥ 12 MB/s |
...
The negative results in Section 3 do not change the security conclusion. If anything, they strengthen it: an entropy source whose mechanism was not fully understood until recently is harder to characterise as an adversarial target, and the co-resident threat model does not depend on which variants pass or fail.
...
1. **Full one-hundred-slice polling-mode measurement.** The nine-slice sample establishes the shape of the polling mode's distribution; a full run would establish its tails. The collapse slice at `H_bitstring = 0.132` may be a rare event or the leading edge of a larger population. Reproducing it is the natural next step.
...
The experimenter designed the generator, proposed all four variants, ran every measurement, and observed every phenomenon reported here. The AI collaborator did not execute code, did not observe any of the hosts, and relied entirely on reported results.
...
Default settings (`skip = 1`, 1 ms warm-up):

```
./target/release/seedy64-threaded --emit-yield 1 1000000 > stream.bin
```
...
Big-endian output for network-ordered pipelines:

```
./target/release/seedy64-threaded --emit-yield --big-endian 1 1000000 > stream.bin
```
...
### B.4 Reproducing the negative controls
